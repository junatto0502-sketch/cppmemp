...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.url_edit.setPlaceholderText("URL / 文字列（クリックでコピー）")
        ...
        form.addWidget(QLabel("内容"))
        form.addWidget(self.url_edit, 5)
        form.addWidget(self.add_btn)
        layout.addLayout(form)

        self.table = QTableWidget(0, 4)
        self.table.setHorizontalHeaderLabels(["ID", "タイトル", "内容", "操作"])
    ...
    def reload(self):
        ...
        for r, (memo_id, title, url) in enumerate(rows):
    ...
    def on_cell_clicked(self, row: int, col: int):
        ...
        title, url = record
        value = {1: title, 2: url}.get(col) or ""
        if value == "":
            return
